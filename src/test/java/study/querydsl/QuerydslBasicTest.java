...
//                .select(member.count()) // count(member.id)
    ...
    @DisplayName("팀의 이름 각 팀의 평균 연령을 구한다.")
    @Test
    ...
    @DisplayName("팀 A에 소속된 모든 회원을 찾는다.")
    @Test
    void join() {
        List<Member> result = queryFactory
                .selectFrom(member)
    ...
    @DisplayName("세타 조인 - 회원 이름이 팀 이름과 같은 회원 조회")
    @Test
    ...
    @DisplayName("회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조회, 회원은 모두 조회한다.")
    @Test
    /**
     * JPQL:
     * select m, t
     * from Member m left join m.team t on t.name = 'teamA'
     */
    ...
    @DisplayName("연관 관계가 없는 엔티 외부 조인 - 회원 이름이 팀 이름과 같은 대상 외부 조인")
    @Test
